<?php 
/**
 * Userlevelpermissions Page Controller
 * @category  Controller
 */
class UserlevelpermissionsController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				userlevelpermissions.userlevelid LIKE ? OR 
				userlevelpermissions.tablename LIKE ? OR 
				userlevelpermissions.permission LIKE ?
			)";
			...
			$this->view->search_template = "userlevelpermissions/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("userlevelpermissions.userlevelid", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Userlevelpermissions";
		...
		$this->render_view("userlevelpermissions/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$this->redirect("userlevelpermissions");
	}
	/**
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("userlevelpermissions.userlevelid", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("userlevelpermissions/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["userlevelid","tablename","permission"];
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'userlevelid' => 'required|numeric',
				'tablename' => 'required',
				'permission' => 'required|numeric',
			);

			$this->sanitize_array = array(
				'userlevelid' => 'sanitize_string',
				'tablename' => 'sanitize_string',
				'permission' => 'sanitize_string',
			);
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("userlevelpermissions");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("userlevelpermissions/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("userlevelpermissions.userlevelid", $rec_id);;
		...
		return $this->render_view("userlevelpermissions/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("userlevelpermissions.userlevelid", $arr_rec_id, "in");
