<?php 
/**
 * Rpt_grupo Page Controller
 * @category  Controller
 */
class Rpt_grupoController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "rpt_grupo";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				rpt_grupo.IdGrupo LIKE ? OR 
				rpt_grupo.DescripcionGrupo LIKE ? OR 
				rpt_grupo.Orden LIKE ?
			)";
			...
			$this->view->search_template = "rpt_grupo/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("rpt_grupo.IdGrupo", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Rpt Grupo";
		...
		$this->render_view("rpt_grupo/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("rpt_grupo.IdGrupo", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("rpt_grupo/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["DescripcionGrupo","Orden"];
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'DescripcionGrupo' => 'required',
				'Orden' => 'required|numeric',
			);

			$this->sanitize_array = array(
				'DescripcionGrupo' => 'sanitize_string',
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("rpt_grupo");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("rpt_grupo/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["IdGrupo","DescripcionGrupo","Orden"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("rpt_grupo.IdGrupo", $rec_id);;
		...
		return $this->render_view("rpt_grupo/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("rpt_grupo.IdGrupo", $arr_rec_id, "in");
