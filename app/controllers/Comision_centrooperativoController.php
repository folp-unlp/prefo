<?php 
/**
 * Comision_centrooperativo Page Controller
 * @category  Controller
 */
class Comision_centrooperativoController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "comision_centrooperativo";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				comision_centrooperativo.idrelacion LIKE ? OR 
				comision_centrooperativo.idComision LIKE ? OR 
				comision_centrooperativo.idCentroOperativo LIKE ?
			)";
			...
			$this->view->search_template = "comision_centrooperativo/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("comision_centrooperativo.idrelacion", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Comision Centrooperativo";
		...
		$this->render_view("comision_centrooperativo/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$this->redirect("comision_centrooperativo");
	}
	/**
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("comision_centrooperativo.idrelacion", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("comision_centrooperativo/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["idComision","idCentroOperativo"];
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'idComision' => 'required|numeric',
				'idCentroOperativo' => 'required|numeric',
			);
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("comision_centrooperativo");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("comision_centrooperativo/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["idrelacion","idComision","idCentroOperativo"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("comision_centrooperativo.idrelacion", $rec_id);;
		...
		return $this->render_view("comision_centrooperativo/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("comision_centrooperativo.idrelacion", $arr_rec_id, "in");
