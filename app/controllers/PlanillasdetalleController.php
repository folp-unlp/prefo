<?php 
/**
 * Planillasdetalle Page Controller
 * @category  Controller
 */
class PlanillasdetalleController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "planillasdetalle";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				planillasdetalle.iddetalle LIKE ? OR 
				planillasdetalle.idPlanilla LIKE ? OR 
				planillasdetalle.idPAciente LIKE ? OR 
				planillasdetalle.TipoPaciente LIKE ? OR 
				planillasdetalle.Sector LIKE ? OR 
				planillasdetalle.Pieza LIKE ? OR 
				planillasdetalle.Diagnostico LIKE ? OR 
				planillasdetalle.Tratamiento LIKE ? OR 
				planillasdetalle.UOI LIKE ?
			)";
			...
			$this->view->search_template = "planillasdetalle/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("planillasdetalle.iddetalle", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Planillasdetalle";
		...
		$this->render_view("planillasdetalle/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$this->redirect("planillasdetalle");
	}
	/**
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("planillasdetalle.iddetalle", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("planillasdetalle/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["idPlanilla","idPAciente","TipoPaciente","Sector","Pieza","Diagnostico","Tratamiento","UOI"];
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("planillasdetalle");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("planillasdetalle/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("planillasdetalle.iddetalle", $rec_id);;
		...
		return $this->render_view("planillasdetalle/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("planillasdetalle.iddetalle", $arr_rec_id, "in");
