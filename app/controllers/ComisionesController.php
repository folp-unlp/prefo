...
class ComisionesController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				comisiones.CicloLectivo LIKE ? OR 
				comisiones.idComision LIKE ? OR 
				comisiones.Asignatura LIKE ? OR 
				comisiones.NumeroComision LIKE ? OR 
				comisiones.Dia LIKE ? OR 
				comisiones.Horario LIKE ?
			)";
			...
			$this->view->search_template = "comisiones/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("comisiones.idComision", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Comisiones";
		...
		$this->render_view("comisiones/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("comisiones.idComision", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("comisiones/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["CicloLectivo","Asignatura","NumeroComision","Dia","Horario"];
			...
			$this->sanitize_array = array(
				...
				'NumeroComision' => 'sanitize_string',
				'Dia' => 'sanitize_string',
        ...
        $this->render_view("comisiones/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["CicloLectivo","idComision","Asignatura","NumeroComision","Dia","Horario"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("comisiones.idComision", $rec_id);;
		...
		return $this->render_view("comisiones/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("comisiones.idComision", $arr_rec_id, "in");
