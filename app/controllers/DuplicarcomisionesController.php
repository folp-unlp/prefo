<?php 
/**
 * Duplicarcomisiones Page Controller
 * @category  Controller
 */
class DuplicarcomisionesController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "duplicarcomisiones";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				duplicarcomisiones.id LIKE ? OR 
				duplicarcomisiones.Fecha LIKE ? OR 
				duplicarcomisiones.CicloLectivo LIKE ? OR 
				duplicarcomisiones.Asignatura LIKE ? OR 
				duplicarcomisiones.Origen LIKE ? OR 
				duplicarcomisiones.Destino1 LIKE ? OR 
				duplicarcomisiones.Destino2 LIKE ? OR 
				duplicarcomisiones.Destino3 LIKE ? OR 
				duplicarcomisiones.Destino4 LIKE ? OR 
				duplicarcomisiones.Destino5 LIKE ?
			)";
			...
			$this->view->search_template = "duplicarcomisiones/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("duplicarcomisiones.id", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Duplicarcomisiones";
		...
		$this->render_view("duplicarcomisiones/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$this->redirect("duplicarcomisiones");
	}
	/**
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("duplicarcomisiones.id", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("duplicarcomisiones/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["Fecha","CicloLectivo","Asignatura","Origen","Destino1","Destino2","Destino3","Destino4","Destino5"];
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'Fecha' => 'required',
				'CicloLectivo' => 'required|numeric',
				'Asignatura' => 'required|numeric',
				'Origen' => 'required|numeric',
				'Destino1' => 'required|numeric',
				'Destino2' => 'required|numeric',
				'Destino3' => 'required|numeric',
				'Destino4' => 'required|numeric',
				'Destino5' => 'required|numeric',
			);

			$this->sanitize_array = array(
				'Fecha' => 'sanitize_string',
				'CicloLectivo' => 'sanitize_string',
				'Asignatura' => 'sanitize_string',
				'Origen' => 'sanitize_string',
				'Destino1' => 'sanitize_string',
				'Destino2' => 'sanitize_string',
				'Destino3' => 'sanitize_string',
				'Destino4' => 'sanitize_string',
				'Destino5' => 'sanitize_string',
			);
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("duplicarcomisiones");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("duplicarcomisiones/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["id","Fecha","CicloLectivo","Asignatura","Origen","Destino1","Destino2","Destino3","Destino4","Destino5"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("duplicarcomisiones.id", $rec_id);;
		...
		return $this->render_view("duplicarcomisiones/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("duplicarcomisiones.id", $arr_rec_id, "in");
