...
class UserlevelsController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				userlevels.userlevelid LIKE ? OR 
				userlevels.userlevelname LIKE ?
			)";
			...
			$this->view->search_template = "userlevels/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("userlevels.userlevelid", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Userlevels";
		...
		$this->render_view("userlevels/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("userlevels.userlevelid", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("userlevels/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["userlevelid","userlevelname"];
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("userlevels");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("userlevels/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("userlevels.userlevelid", $rec_id);;
		...
		return $this->render_view("userlevels/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("userlevels.userlevelid", $arr_rec_id, "in");
