<?php 
/**
 * Comision_docente Page Controller
 * @category  Controller
 */
class Comision_docenteController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "comision_docente";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				comision_docente.IdRelacion LIKE ? OR 
				comision_docente.idComision LIKE ? OR 
				comision_docente.idUsuario LIKE ?
			)";
			...
			$this->view->search_template = "comision_docente/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("comision_docente.IdRelacion", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Comision Docente";
		...
		$this->render_view("comision_docente/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$this->redirect("comision_docente");
	}
	/**
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("comision_docente.IdRelacion", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("comision_docente/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["idComision","idUsuario"];
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("comision_docente");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("comision_docente/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["IdRelacion","idComision","idUsuario"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("comision_docente.IdRelacion", $rec_id);;
		...
		return $this->render_view("comision_docente/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("comision_docente.IdRelacion", $arr_rec_id, "in");
