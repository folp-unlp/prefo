<?php 
/**
 * Tciclolectivo Page Controller
 * @category  Controller
 */
class TciclolectivoController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "tciclolectivo";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				tciclolectivo.CicloLectivo LIKE ?
			)";
			...
			$this->view->search_template = "tciclolectivo/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("tciclolectivo.CicloLectivo", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Tciclolectivo";
		...
		$this->render_view("tciclolectivo/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("tciclolectivo.CicloLectivo", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("tciclolectivo/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["CicloLectivo"];
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("tciclolectivo");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("tciclolectivo/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("tciclolectivo.CicloLectivo", $rec_id);;
		...
		return $this->render_view("tciclolectivo/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("tciclolectivo.CicloLectivo", $arr_rec_id, "in");
