<?php 
/**
 * Rpt_grupotratam Page Controller
 * @category  Controller
 */
class Rpt_grupotratamController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "rpt_grupotratam";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				rpt_grupotratam.idRelacion LIKE ? OR 
				rpt_grupotratam.idgrupo LIKE ? OR 
				rpt_grupotratam.idTratamiento LIKE ? OR 
				rpt_grupotratam.Orden LIKE ?
			)";
			...
			$this->view->search_template = "rpt_grupotratam/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("rpt_grupotratam.idRelacion", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Rpt Grupotratam";
		...
		$this->render_view("rpt_grupotratam/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("rpt_grupotratam.idRelacion", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("rpt_grupotratam/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["idgrupo","idTratamiento","Orden"];
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'idgrupo' => 'required|numeric',
				'idTratamiento' => 'required|numeric',
				'Orden' => 'required|numeric',
			);

			$this->sanitize_array = array(
				'idgrupo' => 'sanitize_string',
				'idTratamiento' => 'sanitize_string',
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("rpt_grupotratam");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("rpt_grupotratam/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["idRelacion","idgrupo","idTratamiento","Orden"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("rpt_grupotratam.idRelacion", $rec_id);;
		...
		return $this->render_view("rpt_grupotratam/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("rpt_grupotratam.idRelacion", $arr_rec_id, "in");
