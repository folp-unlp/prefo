<?php 
/**
 * Tipopaciente Page Controller
 * @category  Controller
 */
class TipopacienteController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "tipopaciente";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				tipopaciente.IdTipoPaciente LIKE ? OR 
				tipopaciente.Descripcion LIKE ? OR 
				tipopaciente.Orden LIKE ?
			)";
			...
			$this->view->search_template = "tipopaciente/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("tipopaciente.IdTipoPaciente", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Tipopaciente";
		...
		$this->render_view("tipopaciente/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("tipopaciente.IdTipoPaciente", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("tipopaciente/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["Descripcion","Orden"];
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("tipopaciente");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("tipopaciente/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["IdTipoPaciente","Descripcion","Orden"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("tipopaciente.IdTipoPaciente", $rec_id);;
		...
		return $this->render_view("tipopaciente/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("tipopaciente.IdTipoPaciente", $arr_rec_id, "in");
