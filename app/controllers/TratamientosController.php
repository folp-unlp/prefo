...
class TratamientosController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				tratamientos.idtratamiento LIKE ? OR 
				tratamientos.Codigo LIKE ? OR 
				tratamientos.Descripcion LIKE ? OR 
				tratamientos.UOI LIKE ? OR 
				tratamientos.Orden LIKE ?
			)";
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("tratamientos.idtratamiento", ORDER_TYPE);
		}
        if($fieldname){
		...
		$this->render_view("tratamientos/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("tratamientos.idtratamiento", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["Codigo","Descripcion","UOI","Orden"];
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["idtratamiento","Codigo","Descripcion","UOI","Orden"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("tratamientos.idtratamiento", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("tratamientos.idtratamiento", $arr_rec_id, "in");
