<?php 
/**
 * T_piezasector Page Controller
 * @category  Controller
 */
class T_piezasectorController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "t_piezasector";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				t_piezasector.Sector LIKE ? OR 
				t_piezasector.pieza LIKE ? OR 
				t_piezasector.Orden LIKE ?
			)";
			...
			$this->view->search_template = "t_piezasector/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("t_piezasector.Sector", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "T Piezasector";
		...
		$this->render_view("t_piezasector/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("t_piezasector.Sector", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("t_piezasector/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["Sector","pieza","Orden"];
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("t_piezasector");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("t_piezasector/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("t_piezasector.Sector", $rec_id);;
		...
		return $this->render_view("t_piezasector/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("t_piezasector.Sector", $arr_rec_id, "in");
