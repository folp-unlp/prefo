<?php 
/**
 * Centrooperativo Page Controller
 * @category  Controller
 */
class CentrooperativoController extends BaseController{
	function __construct(){
		parent::__construct();
		$this->tablename = "centrooperativo";
	}
	/**
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				centrooperativo.idCentroOperativo LIKE ? OR 
				centrooperativo.Nombre LIKE ? OR 
				centrooperativo.Direccion LIKE ? OR 
				centrooperativo.Localidad LIKE ? OR 
				centrooperativo.Partido LIKE ?
			)";
			...
			$this->view->search_template = "centrooperativo/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("centrooperativo.idCentroOperativo", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Centrooperativo";
		...
		$this->render_view("centrooperativo/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("centrooperativo.idCentroOperativo", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("centrooperativo/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["Nombre","Direccion","Localidad","Partido"];
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("centrooperativo");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("centrooperativo/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["idCentroOperativo","Nombre","Direccion","Localidad","Partido"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("centrooperativo.idCentroOperativo", $rec_id);;
		...
		return $this->render_view("centrooperativo/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("centrooperativo.idCentroOperativo", $arr_rec_id, "in");
