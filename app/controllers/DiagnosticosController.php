...
class DiagnosticosController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				diagnosticos.idDiagnostico LIKE ? OR 
				diagnosticos.CodigoDiagnostico LIKE ? OR 
				diagnosticos.Descripcion LIKE ? OR 
				diagnosticos.Orden LIKE ?
			)";
			...
			$this->view->search_template = "diagnosticos/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("diagnosticos.idDiagnostico", ORDER_TYPE);
		}
        if($fieldname){
		...
		$page_title = $this->view->page_title = "Diagnosticos";
		...
		$this->render_view("diagnosticos/list.php", $data); //render the full page
	}
	/**
     * Load csv|json data
     * @return data
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("diagnosticos.idDiagnostico", $rec_id);; //select record based on primary key
		}
		...
		return $this->render_view("diagnosticos/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$tablename = $this->tablename;
			$request = $this->request;
			$fields = $this->fields = ["CodigoDiagnostico","Descripcion","Orden"];
			...
			$this->sanitize_array = array(
				'CodigoDiagnostico' => 'sanitize_string',
			...
			if($this->validated()){
                ...
                if($rec_id) {
                	...
                	return $this->redirect("diagnosticos");
				} else {
					$this->set_page_error();
				}
			}
        ...
        $this->render_view("diagnosticos/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename;
		$fields = $this->fields = ["idDiagnostico","CodigoDiagnostico","Descripcion","Orden"];
		...
		if($formdata){
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("diagnosticos.idDiagnostico", $rec_id);;
		...
		return $this->render_view("diagnosticos/edit.php", $data);
	}
	/**
	...
	function delete($rec_id = null){
		...
		$db->where("diagnosticos.idDiagnostico", $arr_rec_id, "in");
