...
function showToastSuccess(msg) {
    var flashAlert = $(
        '<div class="alert alert-success animated bounce fixed-alert bottom-left"><button type="button" class="close" data-dismiss="alert">&times;</button>' +
        msg +
        "</div>"
    );
...
function showToastDanger(msg) {
    var flashAlert = $(
        '<div class="alert alert-danger animated bounce fixed-alert bottom-left"><button type="button" class="close" data-dismiss="alert">&times;</button>' +
        msg +
        "</div>"
    );
    $("body").append(flashAlert);
...
$(document).on("click", ".optioncheck, .toggle-check-all", function () {
    var sel_ids = $(this)
        .closest(".page")
        .find("input.optioncheck:checkbox:checked")
        .map(function () {
            return $(this).val();
        })
        .get();
...
$(document).on("click", ".open-page-inline", function (e) {
    ...
    if ($(this).closest("tr").length != 0) {
        ...
        if (!loaded) {
            ...
            var newRow = $(
                '<tr class="child-row"><td colspan="' +
                colspan +
                '"><div class="row justify-content-center"><div class="col-md-6"><div class="content reset-grids inline-page">' +
            ...
            newRow
                .find(".content")
                .load(dataURL, function (responseText, status, req) {
                    if (status == "error") {
...
$(document).on("click", ".btn-add-table-row", function (e) {
    ...
    if (rowCount < maxRow) {
        ...
        trTemplate = trTemplate.replace(/row1/gi, "row" + nextRow);
...
$(document).on(
    "submit",
    "form.needs-validation, form.multi-form",
    function (event) {
...
$(document).on("input", ".password-confirm", function (event) {
    ...
    if (value !== match) {
        ...
        inputElem[0].setCustomValidity("Las contraseñas no coinciden");
    } else {
...
function validateForm(formElem) {
    ...
    formElem
        ...
        .css("borderColor", "red");
    formElem
        .find("textarea:required:invalid")
        .parents(".form-group,.input-row")
        .find(".note-editor")
        .css("borderColor", "red");
    ...
    if (confirmPasswordElem.length) {
        ...
        if (value !== match) {
            ...
            confirmPasswordElem[0].setCustomValidity("Las contraseñas no coinciden");
        } else {
            ...
            success: function (result) {
                if (result == true) {
                    ...
                    elemCheckStatus.html(
                        '<small class="text-danger">' + notAvailableMsg + "</small>"
                    );
                } else {
                    inputElem.removeClass("is-invalid").addClass("is-valid");
                    inputElem[0].setCustomValidity("");
                    elemCheckStatus.html(
                        '<small class="text-success">' + availableMsg + "</small>"
                    );
                }
            },
            error: function (err) {
...
$(document).on("change", "[data-load-select-options]", function (e) {
    ...
    arrFields.forEach(function (field) {
        ...
        if (tableRow.length) {
            targetElem = tableRow.find(
                "select[id*=ctrl-" + field + "], input[id*=ctrl-" + field + "]"
            );
        } else {
        ...
        targetElem.html('<option value="">Cargando..</option>');
        var placeholder = targetElem.attr("placeholder") || "Seleccione un valor...";
        $.ajax({
            type: "GET",
            url: path,
            dataType: "json",
...
$(document).on("change", "[data-load-check-options]", function (e) {
    ...
    arrFields.forEach(function (field) {
        ...
        if (tableRow.length) {
            ...
            templateHtml = templateHtml.replace(/row1/gi, "row" + rowIndex);
        } else {
            targetOptionsHolder = $("#" + field + "-options-holder");
        }
        var path =
            targetOptionsHolder.data("load-path") +
            "/" +
            encodeURIComponent(selectedVal);
...
/**
 * Coloca una imágen por defecto cuando no se encuentra una imágen
 */
$(window).bind("load", function () {
    $("img").each(function () {
        if (
            ...
            var noImgSrc = setPathLink("assets/images/no-image.png");
...
function appendPageData(ajaxPage) {
    ...
    if (ajaxPage.data("display-type") == "table") {
        //if page type is tabular
        loading = $(
            '<tr class=""><td class="text-center p-3" colspan="100">' +
            inlineLoadingIndicator +
            "</td></tr>"
        );
    } else {
        loading = $(
            '<div class="text-center p-3 col-12">' + inlineLoadingIndicator + "</div>"
        );
    }
    var pageBody = ajaxPage
        .find("table tbody .page-data, .page-content .page-data")
        .first();
