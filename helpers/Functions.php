...
/**
 * Funciones públicas disponibles desde todo el sistema
 * @version 1.0.0
...
function fixWrongUTF8Encoding($String)
{
    ...
    $UTF8Equivalent = [
        'Ð°', 'Ð±', 'Ð²', 'Ð³', 'Ð´', 'Ðµ', 'Ñ‘', 'Ð¶', 'Ð·', 'Ð¸', 'Ð¹', 'Ðº', 'Ð»', 'Ð¼', 'Ð½', 'Ð¾', 'Ð¿', 'Ñ€', 'Ñ', 'Ñ‚', 'Ñƒ', 'Ñ„', 'Ñ…', 'Ñ†', 'Ñ‡', 'Ñˆ', 'Ñ‰', 'ÑŠ', 'Ñ‹', 'ÑŒ', 'Ñ', 'ÑŽ', 'Ñ', 'Ð', 'Ð‘', 'Ð’', 'Ð“', 'Ð”', 'Ð•', 'Ð', 'Ð–', 'Ð—', 'Ð˜', 'Ð™', 'Ðš', 'Ð›', 'Ðœ', 'Ð', 'Ðž', 'ÐŸ', 'Ð ', 'Ð¡', 'Ð¢', 'Ð£', 'Ð¤', 'Ð¥', 'Ð¦', 'Ð§', 'Ð¨', 'Ð©', 'Ðª', 'Ð«', 'Ð¬', 'Ð­', 'Ð®', 'Ð¯'
    ];
 ...
 * @example redirect_to_page("https://google.com");
 * @return  null
 */
 ...
 * @example <img src="<?php echo set_img_src('uploads/img/89njdh4533.jpg',50,50); ?>" />
 * @return  string
 */
function set_img_src($imgsrc, $width = null, $height = null, $returnindex = 0)
{
    if (!empty($imgsrc)) {
        ...
        $imgpath = "libs/Timthumb.php?src=$src";
 ...
 * @example echo httpGet("http://my_host//phpcurget/");
 * @return string
 */
 ...
 * @example echo http_post("http://my_host/system/phpcurlpost/");
 * @return string
 */
...
/**
 * Devuelve la fecha-hora actual en formato por defecto de Mysql
 * @return  string
 */
...
/**
 * Devuelve la fecha actual en formato por defecto de Mysql
 * @return  string
 */
function date_now()
{
...
/**
 * Devuelve la hora actual en formato por defecto de Mysql
 * @return  string
 */
function time_now()
{
...
/**
 * Devuelve la fecha en un formato deseado
 * @return  string
 */
...
/**
 * Convierte una fecha o un objeto timestamp en una fecha relativa
 * (ej.: 2 días desde ahora, 2 días atrás)
 * @return  string
 */
function relative_date($date)
{
    ...
    // verifica que la fecha proporcionada este formato de fecha unix
    if (is_numeric($date)) {
        $unix_date = $date;
    } else {
    ...
    // verifica si esta en un futuro o es una fecha pasada
    if ($now > $unix_date) {
 ...
 * @return  string
 */
function to_currency($val, $lang = 'es-AR')
{
 ...
 * @return  string
 */
function to_number($val, $lang = 'es')
{
 ...
 * @return  string
 */
function number_to_words($val, $lang = "es")
{
    $f = new NumberFormatter($lang, NumberFormatter::SPELLOUT);
    return $f->format($val);
}
