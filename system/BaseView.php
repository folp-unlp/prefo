 ...
 * Contiene los metodos "render" de las distintas partes que componen la vista
 * y la exportación de los datos en otros formatos (excel, word, json, csv)
 *
...
// libreria para convertir un documento html a pdf
use Dompdf\Dompdf;
...
class BaseView
{
    ...
    /**
     * Renderiza la página en los siguientes formatos [html | json | xml | csv | pdf | word | excel | image]
     * puede ser pasado como cadena en una consulta
     * @var  string
     */
    public $format = "html";

    /**
     * Cantidad máx. de registros para paginar
     * @var  int
     */
    ...
    /**
     * Para renderizar la vista como una vista parcial sin el marco
     * @var boolean
     */
    ...
    public function render($view_name = null, $view_data = null, $layout = "main_layout.php")
    {
        ...
        if (is_ajax()) {
            if (!empty($this->search_template) && file_exists(PARTIALS_DIR . $this->search_template)) {
                include(PARTIALS_DIR . $this->search_template);
            } else {
            ...
            if (file_exists(PARTIALS_DIR . $view_name)) {
                include(PARTIALS_DIR . $view_name);
            } else {
                print_r($view_name);
            }
        }
    ...
    /**
     * Renderiza la vista prinicpal dentro del "layout"
     * @return null
     */
    protected function render_body()
    {
        $view = PARTIALS_DIR . $this->view_name;
        if (file_exists($view)) {
            include($view);
        } else {
    ...
    /**
     * Incluimos una vista dentro de la principal como "partial"
...
<?php
    ...
    private function parse_report_html()
    {
        ...
        // reemplazamos los acentos y dieresis
        //$page_html = html_entity_decode($page_html, ENT_QUOTES, 'UTF-8');
        $page_html = html_entity_decode(preg_replace('~&([a-z]{1,2})(acute|cedil|circ|grave|lig|orn|ring|slash|th|tilde|uml);~i', '$1', htmlentities($page_html, ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8');
        $page_html = str_replace('&', '&amp;', $page_html);
        $page_html = str_replace(array('_lt_', '_gt_', '_amp_'), array('&lt;', '&gt;', '&amp;'), $page_html);
        // Load DOM
        //$orignalLibEntityLoader = libxml_disable_entity_loader(true); DEPRECATED IN PHP8
        $doc = new \DOMDocument();
        @$doc->loadHTML($page_html, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD | LIBXML_NOEMPTYTAG);
        //libxml_disable_entity_loader($orignalLibEntityLoader); DEPRECATED IN PHP8
